...
import pandas as pd
from server.knowledge_base.utils import get_file_content_path, LOADER_DICT
...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    ...
    if selected_kb == "新建知识库":
        with st.form("新建知识库"):
            ...
            logger.info(f"all embed_models:{embed_models}")
            ...
            distance_strategys = ["EUCLIDEAN_DISTANCE", "MAX_INNER_PRODUCT"]
            distance_strategy = cols[2].selectbox(
                "距离算法",
                distance_strategys,
        ...
        with st.expander(
            ...
            chunk_overlap = cols[1].number_input("相邻文本重合长度：", 0, 1000, OVERLAP_SIZE)
            cols[2].write("")
            cols[2].write("")
