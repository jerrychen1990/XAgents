...
from xagents.model.zhipu import GLM


_MODELS = [GLM]
_MODELS_DICT = {model.__name__: model for model in _MODELS}


def list_models() -> list[str]:
    return [e.__name__ for e in _MODELS]


def get_llm_model(config: dict) -> LLM:
    model_cls = config.pop("model_cls")
    model_cls = _MODELS_DICT[model_cls]
